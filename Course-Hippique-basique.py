"""
Pour gérer les mutex on utilise "with nomduverrou" qui fait
un acquire et un release automatiquement

Réalisé par Maximilien REMILLIEUX
et          Jean-Christophe BURNOT
(code de pi à voir chez Maximillieux)
"""
# Cours hippique
...
def un_cheval(ma_ligne : int, keep_running, positions) : # ma_ligne commence à 0
    ...
    while col < LONGEUR_COURSE and keep_running.value :
        ...
        for i in range(3):
            move_to((ma_ligne+1)*3+i,col)         # pour effacer toute ma ligne
...
#%%
def arbitre(positions, Nb_process, LONGEUR_COURSE, keep_running, predict):
    
    while keep_running.value:
        with mutex:
            if max(positions)!=LONGEUR_COURSE-1:
                gagnant = chr(ord('A')+np.argmax(positions))
                move_to((Nb_process*3)+5, 1)
                print("le tank en tête est:", gagnant)
            if min(positions)!=LONGEUR_COURSE-1:
                perdant=chr(ord('A')+np.argmin(positions))
                move_to((Nb_process*3)+6, 1)
                print("le tank en retard est:", perdant)

    print("le gagnant est:", gagnant)
    print("le perdant est:", perdant)
    if predict==gagnant:
...
if __name__ == "__main__" :
    predict=str(input("Pariez sur QQCH (lettre en Majuscule): \n"))
    mutex = mp.Lock()
    ...
    #Gestion de l'arbitre
    arbitre = mp.Process(target = arbitre, args= (positions, Nb_process, LONGEUR_COURSE, keep_running, predict))
    arbitre.start()
    
    move_to((Nb_process*3)+3, 1)
